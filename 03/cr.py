...
model = nb.GaussianNB()
print(ms.cross_val_score(model, x, y, cv=10, scoring='precision_weighted').mean())
print(ms.cross_val_score(model, x, y, cv=10, scoring='recall_weighted').mean())
print(ms.cross_val_score(model, x, y, cv=10, scoring='f1_weighted').mean())
print(ms.cross_val_score(model, x, y, cv=10, scoring='accuracy').mean())
...
mp.xlabel('Predicted class', fontsize=15)
...
for row in range(cm.shape[0]):
    for col in range(cm.shape[1]):
        mp.text(col, row, str(cm[row, col]), color='white',
