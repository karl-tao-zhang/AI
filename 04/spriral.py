...
model_nonc = sc.AgglomerativeClustering(n_clusters=3, linkage='average')
...
model_conn = sc.AgglomerativeClustering(
    n_clusters=3, linkage='average', connectivity=conn)
...
mp.axis('equal')
mp.scatter(x[:, 0], x[:, 1], c=pred_y_nonc, cmap='brg',  s=60)
...
mp.tick_params(labelsize=10)
mp.axis('equal')
mp.scatter(x[:, 0], x[:, 1], c=pred_y_conn, cmap='brg',  s=60)
